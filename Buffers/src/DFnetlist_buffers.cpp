...
/*
 * This file contains the methods used for the insertion of elastic buffers
 * (EBs).
 * The EBs can be inserted in two ways: (1) by explicitly adding blocks with
 * type
 ...
 *
 * The annotated form is meant to be used internally before the netlist is
 * exported
 * into a dot file. For example, the annotations in the channels can be used as
 * a temporary solution before some buffers are retimed for performance
 * optimization.
 *
 * The method instantiateElasticBuffers() is the one that creates EB blocks from
 * the
 ...
 * -------------->| |
 *   |            |M|
 *   |   _ _ _    |U|--> This is a transparent EB with 3 slots (latency = 0 or
 * 1).
 *   |  | | | |   |X|
 *   -->| | | |-->|_|
 *      |_|_|_|
 *
 * The location of the EBs is calculated with an MILP model (inspired in
 * retiming).
 ...
 * are only needed to guarantee elasticity (but not short paths) can be
 * transparent.
 *
*/

void DFnetlist_Impl::createMilpVarsEB(Milp_Model &milp, milpVarsEB &vars,
                                      bool max_throughput, bool first_MG) {
  ...
  ForAllBlocks(b) {
    ...
    ForAllPorts(b, p) {
      ...
      vars.time_elastic[p] =
          milp.newRealVar("timeElastic_" + bname + "_" + pname);
    }
  }
  ...
  vars.in_retime_tokens =
      vector<vector<int>>(MG.size(), vector<int>(vecBlocksSize(), -1));
  vars.out_retime_tokens =
      vector<vector<int>>(MG.size(), vector<int>(vecBlocksSize(), -1));
  vars.th_tokens =
      vector<vector<int>>(MG.size(), vector<int>(vecChannelsSize(), -1));
  vars.retime_bubbles =
      vector<vector<int>>(MG.size(), vector<int>(vecBlocksSize(), -1));
  vars.th_bubbles =
      vector<vector<int>>(MG.size(), vector<int>(vecChannelsSize(), -1));
  ...
  for (int mg = 0; mg < MG.size(); ++mg) {
    ...
    for (blockID b : MG[mg].getBlocks()) {
      ...
      // If the block is combinational, the in/out retiming variables are the
      // same
      vars.out_retime_tokens[mg][b] =
          getLatency(b) > 0 ? milp.newRealVar("outRetimeTok_" + bname)
                            : vars.in_retime_tokens[mg][b];
    }
...
void DFnetlist_Impl::createMilpVarsEB_sc(Milp_Model &milp, milpVarsEB &vars,
                                         bool max_throughput, int mg,
                                         bool first_MG) {
  vars.buffer_flop = vector<int>(vecChannelsSize(), -1);
  vars.buffer_slots = vector<int>(vecChannelsSize(), -1);
  vars.has_buffer = vector<int>(vecChannelsSize(), -1);
  ...
  for (blockID b : MG_disjoint[mg].getBlocks()) {
    ...
    ForAllPorts(b, p) {
      ...
      const string &pname = getPortName(p, false);
      vars.time_path[p] =
          milp.newRealVar("timePath_" + bname + "_" + pname + mg_name);
      vars.time_elastic[p] =
          milp.newRealVar("timeElastic_" + bname + "_" + pname + mg_name);
    }
  }
  ...
  for (blockID b : blocks_in_borders) {
    ...
    ForAllPorts(b, p) {
      ...
      vars.time_path[other_p] =
          milp.newRealVar("timePath_" + bname + "_" + pname_other + mg_name);
      vars.time_elastic[other_p] =
          milp.newRealVar("timeElastic_" + bname + "_" + pname_other + mg_name);
    }
  }
  ...
  for (auto sub_mg : components[mg]) {
    ...
    for (blockID b : MG[sub_mg].getBlocks()) {
      ...
      vars.in_retime_tokens[sub_mg][b] =
          milp.newRealVar("inRetimeTok_" + bname);
      vars.retime_bubbles[sub_mg][b] = milp.newRealVar("retimeBub_" + bname);

      // If the block is combinational, the in/out retiming variables are the
      // same
      vars.out_retime_tokens[sub_mg][b] =
          getLatency(b) > 0 ? milp.newRealVar("outRetimeTok_" + bname)
                            : vars.in_retime_tokens[sub_mg][b];
    }
    ...
    for (channelID c : MG[sub_mg].getChannels()) {
      ...
      // Otherwise, if two channels between same two nodes, milp crashes
      const string cname =
          src + "_" + dst + to_string(getDstPort(c)) + sub_mg_name;
...
void DFnetlist_Impl::createMilpVars_remaining(Milp_Model &milp,
                                              milpVarsEB &vars) {
  vars.buffer_flop = vector<int>(vecChannelsSize(), -1);
  vars.time_path = vector<int>(vecPortsSize(), -1);
  vars.time_elastic = vector<int>(vecPortsSize(), -1);
...
bool DFnetlist_Impl::addElasticBuffers(double Period, double BufferDelay,
                                       bool MaxThroughput, double coverage) {
  Milp_Model milp;
  ...
  if (not createPathConstraints(milp, milpVars, Period, BufferDelay))
    return false;
  if (not createElasticityConstraints(milp, milpVars))
    return false;
  ...
  for (channelID c: buffers) {
      ...
      if (v >= 0) cout << "Throughput " << getChannelName(c) << ": " <<
  milp[milpVars.th_tokens[0][c]] << endl;
  }
  */
...
bool DFnetlist_Impl::addElasticBuffersBB(double Period, double BufferDelay,
                                         bool MaxThroughput, double coverage,
                                         int timeout, bool first_MG) {
  ...
  if (MaxThroughput) {
    ...
    for (int i = 0; i < optimize_num; i++) {
      ...
      cout << fixed << setprecision(2) << milp[milpVars.th_MG[i]] << " ***"
           << endl;
      cout << "************************" << endl;
    }
  }
  ...
  vector<channelID> buffers;
  ForAllChannels(c) {
    if (channelIsCovered(c, false, false, true))
      continue;
...
bool DFnetlist_Impl::addElasticBuffersBB_sc(double Period, double BufferDelay,
                                            bool MaxThroughput, double coverage,
                                            int timeout, bool first_MG) {
  ...
  // Lana 03/05/20 Muxes and merges have internal buffers, fork going to lsq
  // must have one after
  ForAllChannels(c) {
    if ((getBlockType(getSrcBlock(c)) == MUX) ||
        (getBlockType(getSrcBlock(c)) == MERGE &&
         getPorts(getSrcBlock(c), INPUT_PORTS).size() > 1)) {
      setChannelTransparency(c, 1);
      setChannelBufferSize(c, 1);
    }
    if (getBlockType(getSrcBlock(c)) == FORK &&
        getBlockType(getDstBlock(c)) == BRANCH) {
  ...
  for (int i = 0; i < MG_disjoint.size(); i++) {
    ...
    if (not createPathConstraints_sc(milp, milpVars_sc[i], Period, BufferDelay,
                                     i))
      return false;
    ...
    for (channelID c : MG_disjoint[i].getChannels()) {
      ...
      milp.newCostTerm(-1 * order_buf * highest_coef,
                       milpVars_sc[i].has_buffer[c]);
      milp.newCostTerm(-1 * order_slot * highest_coef,
                       milpVars_sc[i].buffer_slots[c]);
    }
    ...
    if (MaxThroughput) {
      for (auto sub_mg : components[i]) {
        ...
        cout << "*** Throughput for MG " << sub_mg << " in disjoint MG " << i
             << ": ";
        cout << fixed << setprecision(2) << milp[milpVars_sc[i].th_MG[sub_mg]]
             << " ***" << endl;
        cout << "************************" << endl;
        if (first_MG)
          break;
    ...
    for (channelID c : buffers) {
      int slots =
          milp[milpVars_sc[i].buffer_slots[c]] + 0.5; // Automatically truncated
...
bool DFnetlist_Impl::createPathConstraints(Milp_Model &milp, milpVarsEB &Vars,
                                           double Period, double BufferDelay) {
  ...
  ForAllChannels(c) {
    ...
    if (getBlockType(getDstBlock(c)) == LSQ ||
        getBlockType(getSrcBlock(c)) == LSQ ||
        getBlockType(getDstBlock(c)) == MC ||
        getBlockType(getSrcBlock(c)) == MC)
      continue;

    int v1 = Vars.time_path[getSrcPort(c)];
    int v2 = Vars.time_path[getDstPort(c)];
    int R = Vars.buffer_flop[c];
    // Jianyi 08.07.21 skip short path done by if statement
    if (v1 == -1 || v2 == -1)
      continue;
    ...
    if (hasPeriod) {
      ...
      // v2 >= v1 - 2*period*R
      if (channelIsCovered(c, false, true, false))
        milp.newRow({{-1, v1}, {1, v2}}, '>',
                    -2 * Period * !isChannelTransparent(c));
      else
  ...
  ForAllBlocks(b) {
    ...
    if (getLatency(b) == 0) {
      ...
      ForAllInputPorts(b, in_p) {
        ...
        if (hasPeriod)
          milp.newRow({{1, v_in}}, '<', Period - d_in);
      }
    }
  }
...
bool DFnetlist_Impl::createPathConstraints_sc(Milp_Model &milp,
                                              milpVarsEB &Vars, double Period,
  ...
  for (channelID c : channels_in_borders) {
    if (!MG_disjoint[mg].hasBlock(getSrcBlock(c)) &&
        !MG_disjoint[mg].hasBlock(getDstBlock(c)))
      continue;
  ...
  for (blockID b : blocks_in_borders) {
    ...
    if (getLatency(b) == 0) {
      ForAllOutputPorts(b, out_p) {
        if (!MG_disjoint[mg].hasBlock(getDstBlock(getConnectedChannel(out_p))))
          continue;

        int v_out = Vars.time_path[out_p];

        ForAllInputPorts(b, in_p) {
          if (!MG_disjoint[mg].hasBlock(getSrcBlock(getConnectedChannel(in_p))))
            continue;
...
bool DFnetlist_Impl::createPathConstraints_remaining(Milp_Model &milp,
                                                     milpVarsEB &Vars,
                                                     double Period,
                                                     double BufferDelay) {
...
bool DFnetlist_Impl::createElasticityConstraints(Milp_Model &milp,
                                                 milpVarsEB &Vars) {

  // Upper bound for the longest rigid path. Blocks are assumed to have unit
  // delay.
  ...
  ForAllChannels(c) {
    ...
    if (channelIsCovered(c, false, true, false)) {
      milp.newRow({{-1, v1}, {1, v2}}, '>',
                  -1 * big_constant * !isChannelTransparent(c));
    } else {
...
bool DFnetlist_Impl::createElasticityConstraints_sc(Milp_Model &milp,
                                                    milpVarsEB &Vars, int mg) {
  // Upper bound for the longest rigid path. Blocks are assumed to have unit
  // delay.
  double big_constant = numBlocks() + 1;
...
bool DFnetlist_Impl::createElasticityConstraints_remaining(Milp_Model &milp,
                                                           milpVarsEB &Vars) {
  // Upper bound for the longest rigid path. Blocks are assumed to have unit
  // delay.
  double big_constant = numBlocks() + 1;
  ...
  ForAllChannels(c) {
    ...
    if (channelIsCovered(c, true, true, false))
      ...
      milp.newRow({{-1, v1}, {1, v2}, {big_constant, Vars.buffer_flop[c]}}, '>',
                  0);
  }
...
bool DFnetlist_Impl::createThroughputConstraints(Milp_Model &milp,
                                                 milpVarsEB &Vars,
                                                 bool first_MG) {
  double frac = 0.5;
  ...
  for (int mg = 0; mg < MG.size(); ++mg) {
    ...
    for (channelID c : MG[mg].getChannels()) {
      ...
      if (getBlockType(getDstBlock(c)) == OPERATOR &&
          getOperation(getDstBlock(c)) == "select_op") {
        if ((getPortType(getDstPort(c)) == TRUE_PORT) &&
            (getTrueFrac(getDstBlock(c)) < frac)) {
          continue;
        }
        if ((getPortType(getDstPort(c)) == FALSE_PORT) &&
            (getTrueFrac(getDstBlock(c)) > frac)) {
          continue;
        }
      }

      // Lana 02.05.20. LSQ serves as FIFO, so we do not need FIFOs on paths to
      // lsq store
      if (getBlockType(getDstBlock(c)) == OPERATOR &&
          getOperation(getDstBlock(c)) == "lsq_store_op")
        continue;
      ...
      milp.newRow({{-1, ret_dst_tok}, {1, ret_src_tok}, {1, th_tok}}, '=',
                  token);
      milp.newRow({{1, th_mg}, {1, hasFlop}, {-1, th_tok}}, '<', 1);

      // There must be registers to handle the tokens and bubbles Slots >=
      // th_tok + th_bub
...
bool DFnetlist_Impl::createThroughputConstraints_sc(Milp_Model &milp,
                                                    milpVarsEB &Vars, int mg,
                                                    bool first_MG) {
  // For every MG, for every channel c

  // if you want to only consider one MG in the throughput, comment the loop and
  // uncomment this line:
  ...
  for (auto sub_mg : components[mg]) {
    ...
    for (channelID c : MG[sub_mg].getChannels()) {
      ...
      int th_tok =
          Vars.th_tokens[sub_mg][c]; // throughput of the channel (tokens)
      // int th_bub = Vars.th_bubbles[mg][c]; // throughput of the channel
      // (bubbles)
      int Slots = Vars.buffer_slots[c];  // Slots in the channel
      int hasFlop = Vars.buffer_flop[c]; // Is there a flop?
      ...
      milp.newRow({{-1, ret_dst_tok}, {1, ret_src_tok}, {1, th_tok}}, '=',
                  token);
      // milp.newRow( {{-1, ret_dst_bub}, {1, ret_src_bub}, {1, th_bub}}, '=',
      // token);
...
void DFnetlist_Impl::makeNonTransparentBuffers() {
  ...
  while (true) {
    ...
    if (SCC.empty())
      return; // No more combinational paths
...
void DFnetlist_Impl::instantiateElasticBuffers() {
  ...
  for (channelID c : ebs) {
    // Lana 03/05/20 Muxes and merges have internal buffers, instantiate only if
    // size/transparency changed by milp
    if ((getBlockType(getSrcBlock(c)) == MUX) ||
        (getBlockType(getSrcBlock(c)) == MERGE &&
         getPorts(getSrcBlock(c), INPUT_PORTS).size() > 1)) {
...
void DFnetlist_Impl::addBorderBuffers() {
  ...
  ForAllChannels(c) {
    ...
    if (getBasicBlock(src) == 0 || getBasicBlock(dst) == 0)
      continue;
    ...
    if (blockIsInMGs(src) ^ blockIsInMGs(dst)) {
      ...
      if (blockIsInMGs(src))
        blocks_in_borders.insert(dst);
      else if (blockIsInMGs(dst))
        blocks_in_borders.insert(src);
...
void DFnetlist_Impl::findMCLSQ_load_channels() {
  ...
  ForAllChannels(c) {
    ...
    if (src_type == MC || src_type == LSQ)
      src_lsq = true;
    if (dst_type == MC || dst_type == LSQ)
      dst_lsq = true;
    if (src_op == "lsq_load_op" || src_op == "mc_load_op")
      src_load = true;
    if (dst_op == "lsq_load_op" || dst_op == "mc_load_op")
      dst_load = true;
    ...
    /*
            ...
            cout << "dst_lsq: " << dst_lsq << ", dst_load: " << dst_load <<
       endl;
    */
...
void DFnetlist_Impl::hideElasticBuffers() {
  ...
  ForAllBlocks(b) {
    if (getBlockType(b) == ELASTIC_BUFFER)
      bls.push_back(b);
  }
...
void DFnetlist_Impl::cleanElasticBuffers() {
  ...
  hideElasticBuffers();
  ForAllChannels(c) {
    // cout << "prev=> buf-size: " << getChannelBufferSize(c) << ", trans: " <<
    // isChannelTransparent(c) << endl;
    setChannelBufferSize(c, 0);
    setChannelTransparency(c, true);
  }
}

void DFnetlist_Impl::setUnitDelays() { ForAllBlocks(b) setBlockDelay(b, 1); }

void DFnetlist_Impl::dumpMilpSolution(const Milp_Model &milp,
                                      const milpVarsEB &vars) const {
  /*    ForAllBlocks(b) {
          ...
          else cout << "In Retiming tok = " << milp[in_ret] << ", Out Retiming
     tok = " << milp[out_ret];
          cout << ", Retiming bub = " << milp[ret_bub] << endl;
      }

      ForAllChannels(c) {
          cout << "Channel " << getChannelName(c) << ": Slots = " <<
     milp[vars.buffer_slots[c]] << ", Flop = " << milp[vars.buffer_flop[c]];
  ...
  */
  ...
  ForAllChannels(c) {
    int v_in = vars.time_path[getSrcPort(c)],
        v_out = vars.time_path[getDstPort(c)];
    if (v_in == -1 || v_out == -1)
      continue;
    cout << getChannelName(c) << endl;
  ...
  ForAllChannels(c) {
    int v_in = vars.time_elastic[getSrcPort(c)],
        v_out = vars.time_elastic[getDstPort(c)];
    if (v_in == -1 || v_out == -1)
      continue;
    cout << getChannelName(c) << endl;
  ...
  for (int mg = 0; mg < MG.size(); mg++) {
    ...
    for (channelID c : MG[mg].getChannels()) {
      ...
      int Slots = vars.buffer_slots[c];
      int hasFlop = vars.buffer_flop[c];
      if (th_tok == -1 || Slots == -1 || hasFlop == -1)
        continue;

      int ret_src_tok = vars.out_retime_tokens[mg][getSrcBlock(c)];
      int ret_dst_tok = vars.in_retime_tokens[mg][getDstBlock(c)];
      if (ret_src_tok == -1 || ret_dst_tok == -1)
        continue;
...
void DFnetlist_Impl::writeRetimingDiffs(const Milp_Model &milp,
