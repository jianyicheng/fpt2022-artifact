...
static bool goodIdentifier(DFnetlist_Impl &DF, const string &name) {
  ...
  for (char c : name) {
    if (isalnum(c) or allowedChars.find(c) != string::npos)
      continue;
    DF.setError("Invalid identifier: " + name + ".");
    return false;
  }
 ...
 */
static bool readPortDeclaration(DFnetlist_Impl &DF, string &name, int &width,
                                PortType &type) {
...
static bool readPorts(DFnetlist_Impl &DF, blockID id, Agnode_t *v, bool input) {
  ...
  if ((attr = agget(v, in_out)) != nullptr) {
    ...
    for (auto &port : tokens) {
      ...
      if (not readPortDeclaration(DF, port, width, type))
        return false;
      portID p = DF.createPort(id, input, port, width, type);
      if (p == invalidDataflowID)
        return false;

      // Lana 04/07/19 Extract memory suffix
      if (s != "")
        DF.setMemPortSuffix(p, s);
    }
  }
...
static bool readLatencyII(DFnetlist_Impl &DF, blockID id, Agnode_t *v) {
  ...
  if (attr != nullptr and strlen(attr) > 0) {
    if (DF.getBlockType(id) != OPERATOR) {
      DF.setError("Block " + block_name +
                  ": latency can only be defined for operators.");
      return false;
    }
  ...
  if (attr != nullptr and strlen(attr) > 0) {
    if (DF.getBlockType(id) != OPERATOR) {
      DF.setError("Block " + block_name +
                  ": II can only be defined for operators.");
      return false;
    }
...
static bool readExecFrequency(DFnetlist_Impl &DF, blockID id, Agnode_t *v) {
  ...
  if (attr != nullptr and strlen(attr) > 0) {
    ...
    if (freq < 0) {
      DF.setError("Block " + block_name +
                  ": wrong value for execution frequency.");
      return false;
    }
...
static bool readTrueFrac(DFnetlist_Impl &DF, blockID id, Agnode_t *v) {
  ...
  if (attr != nullptr and strlen(attr) > 0) {
    ...
    // DF.setError("True/false execution fraction can be specified only for
    // select op.");
    double frac = getPositiveDouble(attr);
    if (frac < 0) {
      DF.setError("Select " + to_string(id) +
                  ": wrong value for execution fraction.");
    }
...
static bool readOperation(DFnetlist_Impl &DF, blockID id, Agnode_t *v) {
  ...
  if (attr != nullptr and strlen(attr) > 0) {
    if (DF.getBlockType(id) != OPERATOR) {
      DF.setError("Block " + block_name +
                  ": operation can only be defined for operators.");
      return false;
    }
...
static bool readBasicBlock(DFnetlist_Impl &DF, blockID id, Agnode_t *v) {
  ...
  //        || (DF.getBlockType(id) == OPERATOR && DF.getOperation(id) ==
  //        "lsq_load_op")
  //        || (DF.getBlockType(id) == OPERATOR && DF.getOperation(id) ==
  //        "lsq_store_op")
  //        || (DF.getBlockType(id) == OPERATOR && DF.getOperation(id) ==
...
static bool readMemName(DFnetlist_Impl &DF, blockID id, Agnode_t *v) {
  ...
  if (attr != nullptr and strlen(attr) > 0) {
    if (DF.getBlockType(id) != MC && DF.getBlockType(id) != LSQ) {
      DF.setError("Block " + block_name +
                  ": memory name can only be defined for MC/LSQ.");
      return false;
    }
...
static bool readLSQParams(DFnetlist_Impl &DF, blockID id, Agnode_t *v) {
  ...
  if (DF.getBlockType(id) == LSQ || DF.getBlockType(id) == SMC_FIFO || DF.getBlockType(id) == SMC_CNTRL) {
...
static bool readGetPtrConst(DFnetlist_Impl &DF, blockID id, Agnode_t *v) {
  ...
  if (DF.getBlockType(id) == OPERATOR) {
    ...
    if (attr != nullptr and strlen(attr) > 0) {
      ...
      if (t < 0) {
        DF.setError("Block " + block_name +
                    ": wrong value for GetPtr constants.");
        return false;
      }
...
static bool readValue(DFnetlist_Impl &DF, blockID id, Agnode_t *v) {
  ...
  if (attr != nullptr and strlen(attr) > 0) {
    if (DF.getBlockType(id) != CONSTANT) {
      DF.setError("Block " + block_name +
                  ": value can only be defined for constants.");
      return false;
    }
...
static bool readBufferAttributes(DFnetlist_Impl &DF, blockID id, Agnode_t *v) {
  ...
  if (attr != nullptr and strlen(attr) > 0) {
    if (DF.getBlockType(id) != ELASTIC_BUFFER) {
      DF.setError("Block " + block_name +
                  ": slots can only be defined for elastic buffers.");
      return false;
    }
    cout << "SHAB: attr for buffer " << DF.getBlockName(id) << ": " << attr
         << endl;
    int slots = getPositiveInteger(attr);
    cout << "SHAB: slots for buffer " << DF.getBlockName(id) << ": " << slots
         << endl;
    if (slots <= 0) {
  ...
  if (attr != nullptr and strlen(attr) > 0) {
    if (DF.getBlockType(id) != ELASTIC_BUFFER) {
      DF.setError("Block " + block_name +
                  ": transparency can only be defined for elastic buffers.");
      return false;
    }

    string str_attr(attr);
    if (str_attr == "true")
      DF.setBufferTransparency(id, true);
    else if (str_attr == "false")
      DF.setBufferTransparency(id, false);
    else {
      DF.setError("Block " + block_name +
                  ": wrong value for transparency (should be true or false.");
      return false;
    }
  }
  return true;
...
// Reads the attributes of elastic buffers (slots and transparency)
static bool readChannelBufferAttributes(DFnetlist_Impl &DF, channelID c,
                                        Agnode_t *v) {
  // cout << "setting channel buffer attributes for " << DF.getChannelName(c) <<
  // endl;
  const char *attr = agget(v, (char *)"slots");
  int slots = 0;
  if (attr != nullptr and strlen(attr) > 0) {
    ...
    if (slots < 0) {
      DF.setError("Channel " + DF.getChannelName(c) +
                  ": wrong value for slots.");
      return false;
    }
...
bool DFnetlist_Impl::readDataflowDot(FILE *f) {
  ...
  if (g == nullptr)
    return false; // No graph has been read.
  ...
  if (not goodIdentifier(*this, net_name))
    return false;
  ...
  for (Agnode_t *v = agfstnode(g); v; v = agnxtnode(g, v)) {
    ...
    if (not validBlock(id))
      return false;

    // Reading input and output ports
    if (not readPorts(*this, id, v, true))
      return false;
    if (not readPorts(*this, id, v, false))
      return false;

    // Reading delays
    if (not readDelays(*this, id, v))
      return false;

    // Reading latency and initiation interval
    if (not readLatencyII(*this, id, v))
      return false;

    // Reading execution frequency
    if (not readExecFrequency(*this, id, v))
      return false;

    // Reading select input fraction
    if (not readTrueFrac(*this, id, v))
      return false;

    // Reading the value for constants
    if (not readValue(*this, id, v))
      return false;

    // Reading the operation for operators
    if (not readOperation(*this, id, v))
      return false;

    // Reading the BB id
    if (not readBasicBlock(*this, id, v))
      return false;

    // Reading mem port param
    if (not readMemPortID(*this, id, v))
      return false;

    // Reading mem port param
    if (not readMemOffset(*this, id, v))
      return false;

    // Reading mem port param
    if (not readMemBBCount(*this, id, v))
      return false;

    // Reading mem port param
    if (not readMemLdCount(*this, id, v))
      return false;

    // Reading mem port param
    if (not readMemStCount(*this, id, v))
      return false;

    if (not readMemName(*this, id, v))
      return false;

    if (not readFuncName(*this, id, v))
      return false;

    if (not readLSQParams(*this, id, v))
      return false;

    // Reading getelementptr array dimensions
    if (not readGetPtrConst(*this, id, v))
      return false;

    // SHAB: to support naive buffer placement
    if (not readBufferAttributes(*this, id, v))
      return false;
  }
  ...
  for (Agnode_t *v = agfstnode(g); v; v = agnxtnode(g, v)) {
    for (Agedge_t *e = agfstout(g, v); e; e = agnxtout(g, e)) {
      ...
      if (not validPort(psrc)) {
        setError("Block " + getBlockName(src) + ": unknown port " +
                 string(attr) + ".");
        return false;
      }
      ...
      if (not validPort(pdst)) {
        setError("Block " + getBlockName(dst) + ": unknown port " +
                 string(attr) + ".");
        return false;
      }
      ...
      if (not readChannelBufferAttributes(*this, c, v))
        return false;
    }
  }
...
bool DFnetlist_Impl::setAllBBFrequencies() {
  for (bbID bb = 1; bb <= BBG.numBasicBlocks(); bb++) {
    ...
    if (BBG.isEntryBB(bb)) {
      ...
      if (succ_freq != pred_freq)
        std::cout << "bb ID = " << bb << ", pred_freq = " << pred_freq
                  << ", succ_freq = " << succ_freq << std::endl;
...
void DFnetlist_Impl::computeChannelFrequencies() {
  ForAllChannels(c) {
    ...
    // inner/outer channel
    if ((src_type == BRANCH &&
        (dst_type == MUX || dst_type == MERGE || dst_type == CNTRL_MG)) ||
         src_type == SMC_CNTRL) {
...
bool DFnetlist_Impl::readDataflowDotBB(FILE *f) {
  ...
  // Name of the graph
  if (not goodIdentifier(*this, string(agnameof(g))))
    return false;
  ...
  for (Agnode_t *v = agfstnode(g); v; v = agnxtnode(g, v)) {
    for (Agedge_t *e = agfstout(g, v); e; e = agnxtout(g, e)) {
      ...
      if (it_dst == name2bbID.end()) {
        setError("Invalid destination Basic Block: " + src_name + " to " +
                 dst_name);
        return false;
      }
      ...
      if (attr != nullptr and strlen(attr) > 0) {
        ...
        if (freq < 0) {
          setError(src_name + "->" + dst_name +
                   " arc: negative execution frequency.");
          return false;
        }
      }
  ...
  // set the entryBB. it is assumed that the entry/exit BB has no input/output
  // edges.
