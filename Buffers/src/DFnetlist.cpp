...
static map<T2, T1> reverseMap(const map<T1, T2> &M12) {
  map<T2, T1> M21;
  for (auto &it : M12)
    M21.emplace(it.second, it.first);
  return M21;
}
...
map<string, BlockType> DFnetlist_Impl::String2BlockType =
    reverseMap<BlockType, string>(DFnetlist_Impl::BlockType2String);
map<string, PortType> DFnetlist_Impl::String2PortType =
    reverseMap<PortType, string>(DFnetlist_Impl::PortType2String);
...
DFnetlist_Impl::DFnetlist_Impl() { init(); }

DFnetlist_Impl::DFnetlist_Impl(const string &name) {
  init();
  if (not readDataflowDot(name))
    return;
  check();
}
...
DFnetlist_Impl::DFnetlist_Impl(const std::string &name,
  ...
  if (not readDataflowDot(name))
    return;
  check();
  if (not readDataflowDotBB(name_bb))
    return;
  // SHAB_note: do error checking
}

const string &DFnetlist_Impl::getName() const { return net_name; }

string DFnetlist_Impl::genBlockName(BlockType type) const {
  string prefix = "_" + BlockType2String[type] + "_";
  int idx = 1;
  while (getBlock(prefix + to_string(idx)) != invalidDataflowID)
    ++idx;
...
blockID DFnetlist_Impl::createBlock(BlockType type, const string &name) {
  ...
  B.latency = 0;
  B.II = 0;
  B.data = B.portCond = B.portFalse = B.portTrue = B.srcCond =
      invalidDataflowID;
  B.slots = 0;
  B.transparent = true;
  B.freq = 0.0;
...
void DFnetlist_Impl::removeBlock(blockID id) {
  ...
  while (not B.allPorts.empty())
    removePort(*(B.allPorts.begin()));
...
bool DFnetlist_Impl::isInnerChannel(channelID c) {
  ...
  if (!((src_bb == dst_bb) || (src_bb == 0) || (dst_bb == 0)))
    cout << getChannelName(c) << ", src_bb = " << src_bb
         << ", dst_bb = " << dst_bb << endl;
...
// Generates a fresh port name for a given block name. isInput indicates whether
// the port
// must be input or output (prefix "in" or "out"). name2port contains all full
// names
// for the ports (to check that not an existing name is generated).
static string genFreshPortName(const string &blockName, bool isInput,
                               map<string, portID> &name2port) {
  string prefix = isInput ? "in" : "out";
  for (int suffix = 1;; ++suffix) {
    string name = prefix + to_string(suffix);
    if (name2port.count(blockName + ":" + name) == 0)
      return name;
  }
}

portID DFnetlist_Impl::createPort(blockID block, bool isInput,
                                  const string &name, int width,
                                  PortType type) {
  assert(validBlock(block));

  string localname =
      name.empty() ? genFreshPortName(getBlockName(block), isInput, name2port)
                   : name;
  ...
  if (isInput)
    B.inPorts.insert(pid);
  else
    B.outPorts.insert(pid);
...
const setPorts &DFnetlist_Impl::getPorts(blockID id, PortDirection dir) const {
  assert(validBlock(id));
  if (dir == INPUT_PORTS)
    return blocks[id].inPorts;
  if (dir == OUTPUT_PORTS)
    return blocks[id].outPorts;
...
portID DFnetlist_Impl::getConnectedPort(portID port) const {
  assert(validPort(port));
  if (isInputPort(port))
    return getSrcPort(getConnectedChannel(port));
  return getDstPort(getConnectedChannel(port));
}

channelID DFnetlist_Impl::createChannel(portID src, portID dst, int slots,
                                        bool transparent) {
...
string DFnetlist_Impl::getChannelName(channelID id, bool full) const {
  ...
  if (full)
    return getPortName(src, true) + " -> " + getPortName(dst, true);
...
void DFnetlist_Impl::setError(const string &err) { error.set(err); }

const string &DFnetlist_Impl::getError() const { return error.get(); }

string &DFnetlist_Impl::getError() { return error.get(); }

void DFnetlist_Impl::clearError() { error.clear(); }

bool DFnetlist_Impl::hasError() const { return error.exists(); }

int DFnetlist_Impl::numBlocks() const { return nBlocks; }

int DFnetlist_Impl::numChannels() const { return nChannels; }

int DFnetlist_Impl::numPorts() const { return nPorts; }
...
bool DFnetlist_Impl::channelIsCovered(channelID c, bool MG, bool borders,
                                      bool MC_LSQ) {
...
bool DFnetlist_Impl::blockIsCovered(blockID b, bool MG, bool borders,
                                    bool MC_LSQ) {
