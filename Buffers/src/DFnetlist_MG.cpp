...
double DFnetlist_Impl::extractMarkedGraphs(double coverage) {
  ...
  while (covered_freq < coverage * total_freq) {
    ...
    for (auto b : newMG.getBlocks())
      min_freq = min(min_freq, freq[b]);
  ...
  cout << "covered_freq = " << covered_freq << ", total_freq = " << total_freq
...
// If frequency of an arc is not set or set to zero or less, it will not be
// included in the MILP.
DFnetlist_Impl::subNetlistBB
DFnetlist_Impl::extractMarkedGraphBB(const map<bbID, double> &freq) {
  Milp_Model milp;
  milp.init();
  ...
  for (bbArcID i = 0; i < BBG.numArcs(); i++) {
    ...
    if (freq.find(i) != freq.end())
      value = (freq.find(i))->second;

    N_e[i] = value;
    N_max = max(N_e[i], N_max);
  }

  vector<int> S_BB(BBG.numBasicBlocks() + 1,
                   -1);                 // Variables to model BB presence
  ...
  for (bbArcID i = 0; i < BBG.numArcs(); i++) {
    if (BBG.getSrcBB(i) == 0 || BBG.getDstBB(i) == 0)
      continue;
    if (N_e[i] <= 0)
      continue;
  ...
  for (bbArcID i = 0; i < BBG.numArcs(); i++) {
    if (S_e[i] >= 0 and milp.isTrue(S_e[i])) {
      ...
      cout << "\t" << BBG.getSrcBB(i) << "->" << BBG.getDstBB(i) << ":"
           << (freq.find(i))->second << endl;
    }
  }
...
DFnetlist_Impl::subNetlist
DFnetlist_Impl::extractMarkedGraph(const map<blockID, double> &freq) {
  ...
  ForAllBlocks(b) {
    ...
    case CONSTANT:
      ...
      ForAllInputPorts(b, p) {
        ...
        if ((channelMG[c] >= 0) && getBlockType(bb) != MC &&
            getBlockType(bb) != LSQ)
    ...
    case MUX:
      // The number of selected non-condition inputs and output channels must be
      // the same
    ...
    case BRANCH:
      ...
      // The input ports must be selected iff one of the output ports is
      // selected.
      inVars.clear();
      outVars.clear();
      // cout <<"-=====\n";
      ForAllOutputPorts(b, p) {
        ...
        if (channelMG[c] >= 0)
          outVars.push_back(channelMG[c]);
        // Lana 30.04.19. Removing sink node check (modified under SINK below)
        // blockID bb = getDstBlock(c);
        // if (channelMG[c] >= 0 && getBlockType(bb) == SINK) milp.equalSum({0},
        // {channelMG[c]});
      }
      ForAllInputPorts(b, p) {
        channelID c = getConnectedChannel(p);
        if (channelMG[c] >= 0)
          milp.equalSum(outVars, {channelMG[c]});
      }
      ForAllBlocks(x) {
    ...
    case SOURCE:
      ...
      break;
    case SINK:
      // Lana 30.04.19 Explicitly mark sink input as non-MG when predecessor
      // MERGE
  ...
  ForAllBlocks(b) {
    ...
    if (not ch.empty()) {
      ...
      // cout << "Block " << getBlockName(b) << ": var " << blocks[b].inMG <<
      // endl;
  ...
  if (freq.empty()) {
    ForAllBlocks(b) {
      ...
      if (v >= 0)
        milp.newCostTerm(1.0, v);
    }
  } else {
    for (auto it : freq) {
      int v = blockMG[it.first];
      if (v >= 0)
        milp.newCostTerm(it.second, v);
    }
  }
  ...
  */
  ...
  ForAllChannels(c) {
    int v = channelMG[c];
    if (v >= 0 and milp.isTrue(v))
      MG.insertChannel(*this, c);
  }
  ...
  /*
      ...
      for (auto const &con: connections) {
          cout << "adding basic block arc " << con.first << "->" << con.second
     << endl;
      }
      cout << "----------------" << endl;
...
DFnetlist_Impl::subNetlist DFnetlist_Impl::getMGfromCFDFC(
    Dataflow::DFnetlist_Impl::subNetlistBB &BB_CFDFC) {
  subNetlist snl;
  ...
  ForAllChannels(i) {
    ...
    if (!BB_CFDFC.hasBasicBlock(src))
      continue;
    if (!BB_CFDFC.hasBasicBlock(dst))
      continue;

    // two cases: 1. outer channel (between two separate BBs or back arc) 2.
    // inner channel (within BB)
    if (!isInnerChannel(i)) {
