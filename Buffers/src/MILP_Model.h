...
public:
  ...
  // SHAB_ans: the pair literally mean: coefficient * vecVars[var index]. like
  // 2a, 3x, etc.
  using Term =
      pair<double, int>; /// Type to represent a term (coefficient, var index)
  ...
  int numConstraints() const { return Matrix.size(); }
   ...
   */
  int newRealVar(string name = "", double lower_bound = 0.0,
  ...
  * @return The index of the variable.
  */
  int newIntegerVar(string name = "", double lower_bound = 0,
  ...
  * @return The index of the variable.
  */
  int newBooleanVar(string name = "") { return newVar(name, BOOLEAN, 0, 1); }
   ...
   */
  bool isFalse(int i) const { return not isTrue(i); }
   ...
   * @return The index of the row.
   */
  int newRow(const vecTerms &terms, char type = '<', double rhs = 0.0,
             const string &name = "") {
    int r = newRow(type, rhs, name);
    for (auto t : terms)
      newTerm(r, t.first, t.second);
    return r;
  }
  ...
  void newTerm(int rowIndex, double coeff, int varIndex) {
    ...
    assert(rowIndex >= 0 and rowIndex < Matrix.size() and varIndex >= 0 and
           varIndex < Vars.size());
  ...
  */
  ...
  bool solve(int timelimit = -1) {
    ...
    if (solver != "cbc" and solver != "glpsol") {
    ...
    string outfile = createTempFilename("MILP_solution", ".gsol");
    string command =
        writeCommand(solver, lpfile, outfile, timelimit) + " >/dev/null 2>&1";
    writeLP(lpfile);
    ...
    if (solver == "cbc")
      readCbcSolution(outfile);
    else if (solver == "glpsol")
      readGlpsolSolution(outfile);
    else
      assert(false);
...
private:
  ...
  vector<int>
      appearanceOrder;   /// Vector to store the indices in order of appearance
  ...
  bool find_solver(const string &s) {
    ...
    if (not s.empty() and s != "cbc" and s != "glpsol") {
      setError("Unkonwn solver " + solver + ".");
      return false;
    }
   ...
   */
  int newVar(const string &name, VarType type, double lower_bound,
             double upper_bound) {
  ...
  void normalizeRow(vecTerms &r) {
    ...
    sort(r.begin(), r.end(),
         [](Term &t1, Term &t2) { return t1.second < t2.second; });
   ...
   */
  string writeCommand(const string &solver, const string &lpfile,
                      const string &solfile, int timeout = -1) {
