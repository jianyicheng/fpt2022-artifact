...
bool skipNodePrint(ENode* enode) {
    ...
    if (enode->type == Branch_ && enode->JustCntrlPreds->size() != 1)
        return true;
    if (enode->type == Join_ && enode->JustCntrlPreds->size() == 1) // if join has only one input it's same as a wire
        return true;
...
int getInPortSize(ENode* enode, int index) {
    ...
    if (enode->type == Argument_) {
        ...
        return CONTROL_SIZE;
    } else if (enode->type == Join_) {
        return CONTROL_SIZE;
    } else if (enode->type == Fifosmc_) {
        return CONTROL_SIZE;
    } else if (enode->type == Selsmc_) {
        return CONTROL_SIZE;
    } else if (enode->type == Cntrlsmc_) {
        return CONTROL_SIZE;
        ...
        case Selsmc_:
            ...
            for (int i = 0; i < (int)enode->CntrlPreds->size() + (int)enode->JustCntrlPreds->size(); i++)
            {
                ...
                if (enode->type == Inst_)
                {
                    ...
                    // && ((int)enode->CntrlPreds->size() + (int)enode->JustCntrlPreds->size()) == 1)
                    // {
                    //     name += "in2:32 ";
                    // }
                    if(enode->isLoadOrStore() && i == 2)
                    {
                        name += "in3:0";
                        continue;
                    }
            ...
            for (auto& pred : *enode->CntrlPreds) {
                if (pred->type != Cst_ && pred->type != LSQ_) {
                    if (pred->Instr->getOpcode() == Instruction::Store) {
                        ...
                        if (is_address)
                        {
                            name += "in" + to_string(getDotAdrIndex(pred, enode)) + ":" +
                                to_string(DATA_SIZE);
                            name += "*s" + to_string(pred->memPortId) + "a ";
                            is_address = false;
                            continue;
                        } else
                        {
                    ...
                    }else if (pred->Instr->getOpcode() == Instruction::Load) {
                        name += "in" + to_string(getDotDataIndex(pred, enode)) + ":" +
                                to_string(DATA_SIZE);
...
std::string getNodeDotOutputs(ENode* enode) {
    ...
    switch (enode->type) {
        ...
        // case Fifosmc_:
            ...
            if (enode->type == Inst_)
            {
                if (enode->isLoadOrStore())
                {
                    ...
                    {
                        for (int i = 0; i < (int)enode->CntrlSuccs->size(); i++)
                        {
                            name += "out" + to_string(i + 1) + ":";
                            if (enode->CntrlSuccs->at(i)->type == MC_)
                            {
                                name += to_string(ADDR_SIZE) + " ";
                            } else
                            {
                    ...
                    {
                        for (int i = 0; i < (int)enode->JustCntrlSuccs->size(); i++)
                        {
                            name += "out" + to_string(i + 1) + ":";
                            if (enode->JustCntrlSuccs->at(i)->type == MC_)
                            {
                                name += to_string(ADDR_SIZE) + " ";
                            } else
                            {
        ...
        case MC_:
            ...
            if (enode->type == MC_)
            {
                for (int i = 1; i <= (int)enode->CntrlSuccs->size(); i++)
                {
                    name += "out" + to_string(i) + ":" +
                                    to_string(DATA_SIZE);
                    name += "*l" + to_string(enode->CntrlSuccs->at(i-1)->memPortId) + "d ";
                }

                // SMC output ack signals
                for (int i = 1; i <= (int)enode->JustCntrlSuccs->size(); i++)
                {
                    name += "out" + to_string(i + (int)enode->CntrlSuccs->size()) + ":" +
                                    to_string(CONTROL_SIZE);
                    if (enode->JustCntrlSuccs->at(i-1)->type == End_)
                    {
                        name += "*e ";
                    }
                    else 
...
std::string getNodeDotParams(ENode* enode) {
    ...
    switch (enode->type) {
        ...
        case Fifosmc_:
        case Cntrlsmc_:
            name += ", fifoDepth=" + to_string(enode->smcFifoDepth);
            break;
        case Cst_:
...
std::string printColor(ENode* from, ENode* to) {
    ...
    else if (from->type == Phi_c || from->type == Branch_c || from->type == Fork_c || from->type == Join_ ||
             from->type == Fifosmc_ || from->type == Selsmc_ || from->type == Cntrlsmc_|| to->type == Phi_c || to->type == Branch_c ||
             to->type == Fork_c || to->type == Join_ || to->type == Fifosmc_ || to->type == Selsmc_ || to->type == Cntrlsmc_ ||
...
std::string printMemEdges(std::vector<ENode*>* enode_dag) {
    ...
    for (auto& enode : *enode_dag) {
        if (enode->type == MC_ || enode->type == LSQ_) {
            int index = 1;
            for (auto& pred : *(enode->JustCntrlPreds)) {
                if (pred->type == Cst_ || pred->type == Fork_c) {
                ...
                {
                    llvm_unreachable("lsq/mc type has invalid control predecessors.");
                }
            }
            ...
            ENode* tmp = nullptr;
            int repeat_inc = 0;
            for (int i = 0; i < enode->JustCntrlSuccs->size(); i++)
            {
                auto succ = enode->JustCntrlSuccs->at(i);
                if (tmp == succ)
                {
                    repeat_inc ++;
                } else
                ...
                if (succ->type == End_ || succ->type == Join_ || succ->type == Fork_c) {
                    ...
                    auto it = std::find(succ->JustCntrlPreds->begin(), succ->JustCntrlPreds->end(),
                                        enode);
                    int succ_ind = distance(succ->JustCntrlPreds->begin(), it) + 1 + repeat_inc;

                    memstr += ", from = \"out" + to_string(end_ind + i) + "\"";
                    memstr += ", to = \"in" + to_string(succ_ind) + "\"";
                    memstr += "];\n";
                }else
                {
                    llvm_unreachable("lsq/mc type has invalid control successors.");
                }
    ...
    for (auto& bnd : *bbnode_dag) {
            ...
            if (enode->type != Branch_n && enode->BB == bnd->BB) {
                ...
                for (auto& enode_csucc : *(enode->JustCntrlSuccs)) {
                    ...
                    if ((enode_csucc->BB == enode->BB || enode_csucc->type == Sink_) &&
                        ...
                        if (enode_csucc->type == Join_)
                        {
                            toInd = indexOf(enode_csucc->JustCntrlPreds, enode) +
                                    enode_csucc->CntrlPreds->size() + 1;
                        }

                        // if (enode->type == Fifosmc_ && enode_csucc->type == Phi_c) {
                        //     toInd = 2;
                        // }

                        if (enode->type == Join_ && enode_csucc->type == Selsmc_) {
                            toInd = 2;
                        }

                        // if (enode->type == Fork_ && enode_csucc->type == Selsmc_) {
                        //     toInd = 1;
                        // }
                        // if (enode->type == Selsmc_ && enode_csucc->type == Branch_c) {
                        //     toInd = 2;
                        // }
                        if (enode_csucc->isLoadOrStore())
                        {
                            toInd = 3;
                        }
                        str += ", to = \"in" + to_string(toInd) + "\"";
        ...
        for (auto& enode : *enode_dag) {
            if (enode->BB == bnd->BB) {
                ...
                for (auto& enode_csucc : *(enode->JustCntrlSuccs)) {
                    if ((enode->type == Branch_c && enode->BB) || enode->type == Cntrlsmc_) {
                        str += printEdge(enode, enode_csucc);
                        str += " [";
                        str += printColor(enode, enode_csucc);
                        ...
                        if (enode->type == Cntrlsmc_)
                        {
                            fromInd = 1;
                        }

                        if (enode_csucc->type == Cntrlsmc_)
                        {
                            fromInd = 2;
                        }
