                    ...
                    if (nd2->type == Inst_ || nd2->type == Phi_ || nd2->type == Branch_) {
                        ...
                        if (nd2->A == Inst.getOperand(i)) {
                            ...
                            if ((Inst.getOperand(i)->getType()->isArrayTy() ||
                                ...
                                llvm::errs() << "Ignored array argument at " << i << "\n"
...
void CircuitGenerator::removeRedundantAfterElastic(std::vector<ENode*>* enode_dag) {
    ...
    std::cerr << "remove redundant" << std::endl;
    ...
    for (size_t idx = 0; idx < enode_dag->size(); idx++) {
        ...
        {
            ENode* tmp_node = nullptr;
            for (int i = 0; i < enode->JustCntrlSuccs->size(); i++)
            {
                auto curr_node = enode->JustCntrlSuccs->at(i);
                if (tmp_node == curr_node && curr_node->type == Join_)
                {
                    //remove it if redundant connection (current one is same as last one)
                    enode->JustCntrlSuccs->erase(enode->JustCntrlSuccs->begin() + i);
                    auto it = std::find(curr_node->JustCntrlPreds->begin(), curr_node->JustCntrlPreds->end(),
                                        enode);
                    int succ_ind = distance(curr_node->JustCntrlPreds->begin(), it) + 1;
                    curr_node->JustCntrlPreds->erase(curr_node->JustCntrlPreds->begin() + succ_ind);
                    i--;
                } else
                {
                    tmp_node = curr_node;
