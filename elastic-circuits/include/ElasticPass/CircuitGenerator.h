...
public:
    ...
    ENode* findNodeInBB(node_type, BBNode* bbNode);
    ENode* getDataInput(ENode* node);
    void insertBetween(ENode* nodePreds, ENode* nodeSuccs, ENode* insertNode);
    void setId(ENode* node);
    void mergeOrInsertMemoryAck(ENode* InstNode, ENode* memNode, ENode* succ);
    void setLoadOrStoreTriggerInputs(ENode* memNode, std::vector<ENode*>& memoryInst);
    void addMemoryNode(ENode* enode, std::map<const Value*, ENode*>& MemNodesCollect, std::map<ENode*, std::vector<ENode*>>& MemInstCollect);
    void addMemoryNodesConnection();
